...
int hash_table_set(hash_table_t *ht, const char *key, const char *value)
{
	...
	if (ht == NULL || key == NULL || value == NULL || size_key == 0)/* validate input */
		return (0);
	...
	{
		temp = array;
		while (temp != NULL)/* check if the key already exists update the value and free the node y allco*/
		{
			if (strcmp(temp->key, key) == 0)
			{
				if (strcmp(temp->value, value) != 0)/* if the value is different update the value */
				{
					free(temp->value);
